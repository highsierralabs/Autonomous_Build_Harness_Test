...
| `explorer/corpus_adapter/` | builder `corpus_adapter` | `adapter.py` (the retrieval importer of `RHACO_corpus_index` and `RHACO_tool_catalog_librarian`; it is one of three licensed product-tree importers, enumerated in 4.1), `sql.py` (the documented read-only queries D-Q1..D-Q12), `paths.py` (junction-aware confinement), `availability.py` (vector-channel probe) |
...
- `ModeResult` -- `query`, `mode_requested`, `mode_effective` (`identifier` | `lexical` | `hybrid` | `hybrid-degraded-lexical` | `graph` | `graph-degraded-semantic-seed`), `degradation_notice: str | None`, `items: list[ResultItem]`, `truncated`, `total_matched`.
...
| D-Q12 | card by document filename stem (amends resolution, O8) | `SELECT <cards columns> FROM cards WHERE doc_filename LIKE ? ESCAPE '\' ORDER BY yaml_path` -- the bound pattern is the escaped stem plus a literal `.` plus `%`, so `stem` matches `stem.md` but never `stemX.md`. Added round 6 (S8-C4 item 3) to close SCOPE row 17: an `amends` edge's `from_id` is a filename stem, which the doc_id-keyed D-Q3 can never match |
...
### 4.6 `diagnostics`
`GET /diagnostics` and `GET /api/diagnostics`: application version, db path (+ non-authoritative statement), `IndexMeta`, `VectorAvailability` (with reason), active/default mode, degradation state, counts, exclusion sets (module `BODY_DENY_DIRS`, librarian `DENY_DIRS`, read as constants), rerank standing (documented-FAIL; artifact directory present yes/no by `os.path.isdir`, never loaded), last probe run, read as the newest-by-mtime `docs/probe-qualification/runs/*/run_summary.json` (**corrected round 8**: this line named a single `docs/probe-qualification/last_run.json`, a file the probe has never written -- the round-1 diagnostics dispatch specified the `runs/*/run_summary.json` glob and the builder implemented it, so the drift was in this document, not the code. Recorded by the builder as an open issue at round 1, listed by critic round 1 among its overstated claims, and the only sealed-audit defect item both the harness and the critic caught -- SA-5. Neither repaired it for seven rounds because it is ARCHITECTURE text and no builder round owned it), a **Check freshness** action (`POST /diagnostics/freshness` -- the only POST; it computes and displays, writes nothing, O9), and the required authority notice verbatim: **Retrieval is not adjudication.** ...
...
| A22 | **The `#keyhelp` block is integrator-owned duplicated markup, and the key contract is guarded by a static test.** `explorer/web/static/app.js` binds `/`, `?`, `j`, `k`, `Enter`, `Escape`; the identical `#keyhelp` block in all five page templates advertised only `/` and `?`. The integrator updated all five and added `tests/integration/test_keyhelp_contract.py`, which asserts both directions of the contract and fails if any page module carries an unregistered `#keyhelp` block | (a) leave each page module to fix its own block in a later round; (b) move the block into `base.html` as a shared partial, deleting the five copies | PROMPT.md section 4 (the integrator owns shared templates and assets when ownership cannot be isolated); round-3 change request from strand S6-B9, filed because a builder cannot edit another module's template | (a) costs one builder round per page module -- five -- for a documentation edit, and drifts again at the next binding change; (b) is the better end state but `base.html` belongs to the `web` builder, and moving the block mid-wave would invalidate each page's own rendering tests. The static contract test makes (a)'s drift detectable in the meantime, and it immediately caught the newly merged lineage page, which had copied the two-key block | **Director disposition, 2026-09-05 (boundary 3):** the repair stands, because it was disclosed with a compensating control rather than absorbed -- but the defect the test guards is the duplication itself, and the structural fix is a **`web` round-2 candidate, NOT an integrator edit**: byte-identical shell markup belongs in `base.html` once. Logged as an open item visible to the critic, and as ledger item P-13 (ownership defined by cost rather than structure). The contract test remains either way |
...
| A21 | **`base.html` head-injection hook** (round-2 change request from the catalog builder): `{% block head_extra %}{% endblock %}` inside `<head>` after the `app.css` link, so a module template can link its own stylesheet from `<head>`; existing module `<link>` placements (catalog via `{% block scripts %}`, reader inside `{% block content %}`) keep working and migrate at the web builder's wave-3 pass | Leave each module to place its stylesheet wherever `base.html` allows | PROMPT.md section 4 (integrator owns shared templates when ownership cannot be isolated); A19 precedent | A stylesheet linked from the body is functionally correct but not conventional; the hook is additive and inert until used | n/a |
