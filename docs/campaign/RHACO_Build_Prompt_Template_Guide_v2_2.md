...
| `MODULE_LIST` | One noun per independently ownable subsystem — roughly 8–15 in practice. Each becomes a folder, a builder, a showcase mode, a critic target. Group by data ownership. |
...
Fixtures: derive from the production input source's runtime-resolved types, ranges and value shapes; census fixture-vs-production at round 0. The RHACO run's hand-authored fixture quoted every date; 60% of the live corpus did not, and four green rounds could not see the crash.
...
| `CALIBRATION_ARTIFACTS` | Three things of known quality, each scored twice; the spread is the critic's repeatability and the floor under any numeric `round_delta`. Without them the scale is ordinal: say so in SCOPE.md and never read PASS from it. |
...
| Claim audit (5a) | A separate agent after each merge. State in LAYERS.md which claim classes have a mechanical grammar (a citation auditor, a docstring-vs-signature check) and which remain a reader's. Every prose behaviour claim ends up with a named check or an `UNVERIFIED` tag. A mechanical sub-auditor is qualified like the probe — one known-good, one known-bad — before its MATCH counts toward PASS. Clean = zero MISMATCH and zero `UNVERIFIED` on any claim a PASS criterion rests on. |

### Authority and identity block

| Slot | Notes |
|---|---|
| `SUBSTRATE_PATHS` | The corpus, database, service or repository the artifact reads and must never write. Each entry names its closeout invariant in CONSTRAINTS.md (hash equality for a frozen artifact; commit and dirty-path census for a repository; the store's own write audit for a live store or service); verified by a closeout hand, never asserted by the build. |
...
```json
{
  ...
  "modules": {
    "terrain": { "ref_score": 8.1, "round_delta": 0.4, "iso_vs_integrated": 0.0, "rounds": 3, "status": "open",
                 "issues": ["LOD pop at 400 m"], "unverified": ["fits 1280 px as rendered — no browser in strand"] },
    "sky":     { "ref_score": 8.6, "round_delta": 0.1, "iso_vs_integrated": 0.0, "rounds": 2, "status": "pass", "issues": [], "unverified": [] }
  },
  ...
  "claim_audit": { "last_run": "...", "match": 169, "mismatch": 0, "unverifiable": 8, "unverifiable_material": 0 },
  "final_gate": { "whole_ref_score": null, "blind_ab": [] },
  "terminal_state": null
}
```

`unverified` entries are never deleted by the integrator; they are retired only by a cited check. "Resume from the weakest module" is `min(ref_score)` over `status == open`; `unverifiable_material` counts the `UNVERIFIED` entries a PASS criterion rests on and must read 0 at PASS. The "never inflate" rule applies to this file first.
...
| Probe green, product broken on real data | Fixture reproduces the authors' conventions | Derive the fixture from the production input source's runtime-resolved types; census at round 0 |
...
- [ ] Mechanical claim-audit tools qualified on one known-good and one known-bad
- [ ] `SUBSTRATE_PATHS` named, each with its closeout invariant; closeout hand assigned
